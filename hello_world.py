...
foil_end_threshold = st.sidebar.slider("フォイリング終了速度 (km/h)", 0.0, 20.0, 10.0, 0.5,
                                       help="一度フォイリングが始まった後は、この速度を下回るまでフォイリング状態が『継続』します。必ず開始速度以下に設定してください。")
...
if uploaded_file is not None:
    ...
    # 直線区間のラベル上書き（新しいフォイリング判定がTrueの場所）
    ...
    with st.expander("📂 解析データテーブルの表示"):
        st.dataframe(df[['time', 'speed_smooth', 'bearing', 'turn_cum', 'is_foiling']].head(100))
