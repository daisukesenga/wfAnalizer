...
foil_start_threshold = st.sidebar.slider("フォイリング開始速度 (km/h)", 0.0, 20.0, 13.5, 0.5,
                                         help="この速度を超えるとフォイリングが『開始』したとみなします。")
foil_end_threshold = st.sidebar.slider("フォイリング終了速度 (km/h)", 0.0, 20.0, 10.0, 0.5,
                                       help="一度フォイリングが始まった後は、この速度を下回るまでフォイリング状態が『継続』します。")
...
jibe_turn_angle_threshold = st.sidebar.slider("直線とみなす最大角度（度）", 10, 120, 45, 5,
                                              help="この角度以下しか曲がっていない区間を『直線（黄色）』とみなします。")
...
if uploaded_file is not None:
    ...
    # ① フォイリング率の計算
    foiling_ratio = (df['is_foiling'].sum() / len(df)) * 100 if len(df) > 0 else 0
    ...
    # 直線区間のベースマッピング
    df.loc[(df['is_straight']) & (df['is_foiling']), 'segment_type'] = 'straight'

    # --- ⚠️ ご要望の「新・沈（ワイプアウト）」検出ロジック ---
    # ロジック：
    # 状態A: 「フォイリング直線(straight)」を走っている状態から、
    # 状態B: 「非フォイル(is_foiling == False)」に落ち、それが10秒以上「継続」し、
    # 状態C: 再び「フォイリング状態」に戻った（またはログの最後を迎えた）境目を検出する。
    ...
    while i < n - 1:
        # 1. 直線フォイリング中であるポイントを見つける
        if df.loc[i, 'segment_type'] == 'straight':
            # 直線の終わりのインデックスを探す
            j = i
            while j < n and df.loc[j, 'segment_type'] == 'straight':
                j += 1
                
            # 直線が終わった直後、着水（非フォイル）したかチェック
            if j < n and not df.loc[j, 'is_foiling']:
                fall_idx = j
                non_foil_duration = 0
                
                # 非フォイル状態がどこまで続くか追跡
                k = fall_idx
                while k < n and not df.loc[k, 'is_foiling']:
                    k += 1
                
                # kは「再びフォイルした瞬間」または「ログの末尾」
                if k < n:
                    non_foil_duration = (df.loc[k, 'time'] - df.loc[fall_idx, 'time']).total_seconds()
                    # 💡 条件：非フォイル状態が10秒以上続き、その後フォイル状態に戻った場合
                    if non_foil_duration >= 10:
                        wipeout_indices.append(fall_idx) # 着水した瞬間を沈としてマーク
                else:
                    # ログの最後まで戻らなかった（そのまま終了した）場合も、10秒以上あれば沈とする
                    non_foil_duration = (df.loc[n-1, 'time'] - df.loc[fall_idx, 'time']).total_seconds()
                    if non_foil_duration >= 10:
                        wipeout_indices.append(fall_idx)
                        
            i = j  # スキャン位置をスキップ
        else:
            i += 1
            
    wipeouts = df.loc[wipeout_indices] if wipeout_indices else pd.DataFrame()
    ...
    col4.metric("⚠️ 直線からの沈回数", f"{len(wipeouts)} 回")
    ...
    with left_col:
        ...
        # --- 3. 新基準に基づく「沈（ワイプアウト）」ポイントのプロット ---
        if not wipeouts.empty:
            fig_map.add_trace(go.Scattermapbox(
                lat=wipeouts['lat'], lon=wipeouts['lon'],
                mode='markers',
                marker=dict(size=14, color='black', symbol='cross'), # 💡 ご要望通りの×マーク
                name='直線からの沈 (10秒以上非フォイル)',
                hoverinfo='text',
                text=wipeouts['time'].dt.strftime('沈発生時刻: %H:%M:%S')
            ))
    ...
    with right_col:
        ...
        # 速度グラフ上にも「沈」のタイミングを縦赤線で明示
        for w_time in wipeouts['time'] if not wipeouts.empty else []:
