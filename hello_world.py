...
if uploaded_file is not None:
    ...
    # 【ステップ1】まず「ジャイブ区間（成功・失敗）」を確定させてラベルをロックする
    ...
    # 【ステップ2】ジャイブに判定されなかった場所のうち、直線フォイリング中の場所を「直線」とする
    df.loc[(df['segment_type'] == 'normal') & (df['is_straight']) & (df['is_foiling']), 'segment_type'] = 'straight'

    # 【ステップ3】⚠️ 改善された「直線からの沈」検出ロジック
    # ジャイブ（成功・失敗）としてロックされた区間は絶対に上書きしない
    wipeout_indices = []
    i = 0
    n = len(df)
    ...
    while i < n - 1:
        ...
        if df.loc[i, 'segment_type'] == 'straight':
            ...
            # 直線が終わった直後の状態をチェック
            if j < n:
                # 💡【ここを修正】直線が終わった直後が「ジャイブ（success/fail）」なら、それは沈ではなくただのジャイブ動作
                if df.loc[j, 'segment_type'] in ['success', 'fail']:
                    i = j
                    continue
                
                # 直線が終わった直後、ジャイブを挟まずに「非フォイル状態」に落ちた場合
                if not df.loc[j, 'is_foiling']:
                    fall_idx = j
                    
                    # 非フォイル状態がどこまで続くか（または次のフォイル開始かログ終了まで）カウント
                    k = fall_idx
                    ...
                    # 10秒以上非フォイルが続いた場合のみ「直線からの沈」として認定
                    if duration >= 10:
