...
if uploaded_file is not None:
    ...
    # 5秒間で70度以上方位が変わったポイントを「ターン候補」とする
    turns_candidate = df[df['turn_cum'] > 70]
    
    jibe_success_count = 0
    jibe_fail_count = 0
    valid_turn_indices = [] # マップ描画用に「フォイリング中から始まった有効なターン」のインデックスを保持
    ...
    # マップ描画用に有効なターンデータだけを抽出したDataFrameを作成
    ...
    with left_col:
        ...
        # 💡 フォイリング中から始まった有効なターン（valid_turns）のみを描画対象にする
